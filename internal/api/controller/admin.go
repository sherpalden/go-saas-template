...
type AdminController struct {
	...
	adminValidator validator.AdminValidator
...
func (ctrl AdminController) Login(ctx *gin.Context) {
	...
	if validations, passed := validator.GenerateValidation(ctrl.adminValidator, loginCredential); !passed {
		err := app_error.New(errors.New("validation error"), http.StatusBadRequest).SetFieldErrors(validations)
		httpi.HandleError(ctx, err)
		return
	}

	tokens, err := ctrl.adminService.Login(ctx, loginCredential)
	if err != nil {
		httpi.HandleError(ctx, err)
		return
	}
...
func (ctrl AdminController) Refresh(ctx *gin.Context) {
	refreshToken, err := ctx.Cookie("refresh_token")
	if err != nil {
		httpi.HandleError(ctx, err)
		return
	}
	tokens, err := ctrl.adminService.Refresh(ctx, refreshToken)
	if err != nil {
		httpi.HandleError(ctx, err)
		return
	}
...
func (ctrl AdminController) FindAll(ctx *gin.Context) {
	paginationReq := httpi.PaginationRequest{}
	if err := httpi.BuildQueryParams(ctx, &paginationReq); err != nil {
		ctrl.logger.Zap.Errorf("failed to fetch admins: %v", err)
		httpi.HandleError(ctx, err)
		return
	}
	admins, paginationRes, err := ctrl.adminService.FindAll(ctx, paginationReq)
	if err != nil {
		ctrl.logger.Zap.Errorf("failed to fetch admins: %v", err)
		httpi.HandleError(ctx, err)
		return
	}
	httpi.JSONWithMetaData(ctx, http.StatusOK, admins, paginationRes)
}
